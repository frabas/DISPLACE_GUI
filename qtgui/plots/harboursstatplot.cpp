...
void HarboursStatPlot::saveTo()
{
    ...
    QString fn = QFileDialog::getSaveFileName(nullptr, QObject::tr("Save plot data"), QString(),
    ...
    if (!fn.isEmpty()) {
        ...
        if (!file.open(QIODevice::ReadWrite)) {
            QMessageBox::warning(nullptr, QObject::tr("Error"),
                                 QObject::tr("Cannot save to %1: %2").arg(fn).arg(file.errorString()));
            return;
        }
...
HarboursStatPlot::getData(DisplaceModel *model, displace::plot::HarboursStat stat, int harbour)
{
    auto db = model->getOutputStorage();
    if (db == nullptr) {
