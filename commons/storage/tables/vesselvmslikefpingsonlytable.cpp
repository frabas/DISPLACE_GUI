...
void VesselVmsLikeFPingsOnlyTable::queryAllVesselsAtStep(int tstep, std::function<bool(
        const VesselVmsLikeFPingsOnlyTable::Log &)> op)
{
    std::unique_lock<std::mutex> m(p->mutex);

    p->where.bind(tstep);
    p->selectStatement.exec(
            [&op](int id, int tstep, int month, int tstepdep, int nodeid, int popid, int szGroup, double catches,
                  int tstepmax) {
                Log l;
                l.id = id;
                l.tstep = tstep;
