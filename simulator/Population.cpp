...
// for catch equation
    this->set_selected_szgroups(_selected_szgroups); // for the catch equation

// ...then fill in with avai0_beta
    this->set_avai0_beta(_avai0_beta); // set the pop-specific beta from glm for szgroup0

// ...then fill in with avai0_beta
    this->set_avai2_beta(_avai2_beta); // set the pop-specific beta from glm for szgroup2

// ...then fill in with avai0_beta
    this->set_avai3_beta(_avai3_beta); // set the pop-specific beta from glm for szgroup3

// ...then fill in with avai0_beta
    this->set_avai5_beta(_avai5_beta); // set the pop-specific beta from glm for szgroup5

// ...then fill in with avai0_beta
    this->set_avai7_beta(_avai7_beta); // set the pop-specific beta from glm for szgroup7
...
// CALIB: if increased then improve the catch rate of te simulated vessels....
    ...
    this->set_cpue_multiplier(1*a_calib_cpue_multiplier); // set to 1 the first year

// ...then fill in with start pop
    this->set_tot_N_at_szgroup(init_tot_N_at_szgroup); // set the overal N_at_szgroup

// ...then fill in with start pop
    this->set_tot_N_at_szgroup_just_after_redistribution(init_tot_N_at_szgroup); // set the overal N_at_szgroup

// ...then fill in with start pop also for minus 1
    this->set_tot_N_at_szgroup_month_minus_1(init_tot_N_at_szgroup); // set the overall N_at_szgroup

// ...then fill in with start pop also for minus 1
    this->set_tot_N_at_szgroup_year_minus_1(init_tot_N_at_szgroup); // set the overall N_at_szgroup
...
// add these Ns to the multimap of the concerned nodes
    ...
    {
        list_nodes.at(i)->set_Ns_pops_at_szgroup(a_name, tot_N_at_szgroup); // caution: here is tot N on the node! need to call distribute_N()
...
void Population::do_growth()
{
    ...
    vector <double> new_tot_N_at_szgroup (tot_N_at_szgroup.size()); // init
...
void Population::add_recruits_from_SR()
{
    ...
    // compute SSB
    vector <double> SSB_per_szgroup (tot_N_at_szgroup.size()); // init
...
void Population::add_recruits_from_eggs()
{
    ...
    vector <double> recruits_per_szgroup (tot_N_at_szgroup.size()); // init
    for(unsigned int i = 0; i < tot_N_at_szgroup.size(); i++)
    {
...
void Population::compute_tot_N_and_F_and_M_and_W_at_age()
{
    ...
    vector <double> tot_M_at_age (tot_F_at_age.size()); // init
    vector <double> tot_W_at_age (tot_F_at_age.size()); // init
    vector <double> tot_N_at_age (tot_F_at_age.size()); // init
    vector <double> tot_N_at_age_minus_1 (tot_F_at_age.size()); // init
...
double Population::compute_fbar()
{
    ...
    {
        fbar+=this->tot_F_at_age[a]; // sum...
    }
    fbar=fbar/(fbar_ages_min_max.at(1)-fbar_ages_min_max.at(0)); // then do the average...
    return(fbar);
}
...
void Population::compute_TAC()
{
    ...
    double fbar_py= this->compute_fbar(); // at the end of the last year, then for last year py...
    ...
    vector <double> forecast_tot_N_at_age_start_y(tot_N_at_age_end_previous_y.size()); // init
    for (int i=1; i < tot_N_at_age_end_previous_y.size(); i++)
    {
        forecast_tot_N_at_age_start_y.at(i) = tot_N_at_age_end_previous_y.at(i-1); // class change from 31th of Dec to 1st of Jan
    }
    forecast_tot_N_at_age_start_y.at(0) = tot_N_at_age_end_previous_y.at(0);// note: forecast_tot_N_at_age_y.at(0) keeps the same=> assuming fixed recruits...
    ...
    vector <double> forecast_tot_N_at_age_start_y_plus_1 (forecast_tot_N_at_age_start_y.size()); //init
    vector <double> tot_F_at_age_end_previous_y = this->get_tot_F_at_age();
    vector <double> tot_F_at_age_y_plus_1(tot_F_at_age_end_previous_y.size()); //init
    for (int i=0; i < tot_F_at_age_end_previous_y.size(); i++)
    {
        tot_F_at_age_y_plus_1.at(i) = tot_F_at_age_end_previous_y.at(i)*fmultiplier*fmultiplier; // target for y+1
    }
    ...
    // 4. compute the TAC (in tons) according to the forecast N (Baronov´s equation)
    ...
    for (int i=1; i < forecast_tot_N_at_age_start_y_plus_1.size(); i++)
    {
        tac_y_plus_1+=   (tot_F_at_age_y_plus_1.at(i)/((tot_F_at_age_y_plus_1.at(i))+tot_M_at_age_y_plus_1.at(i))) *
                         ...
                         (1- (exp(-(tot_F_at_age_y_plus_1.at(i)+tot_M_at_age_y_plus_1.at(i)))))/ 1000; // convert in tons;
    ...
    double tac_y= this->get_tac()->get_current_tac(); // i.e. at y
    ...
    this->get_tac()->add_tac_y_plus_1(tac_y_plus_1); // the current tac is the avant-dernier
...
// get Mortality
    ...
    {
        new_tot_N_at_szgroup[i] =  tot_N_at_szgroup[i] *exp(-M_at_szgroup[i]/12); // divide according to tstep
...
void Population::export_popdyn_F(ofstream& popdyn_F, int tstep)
{
    ...
    for(unsigned int a = 0; a < tot_F_at_age.size(); a++)
    {
        popdyn_F  << tot_F_at_age.at(a)  << " " ; // output F in CUMUL over months, caution!
    }
    popdyn_F << " " <<  endl;
}
...
void Population::export_popdyn_annual_indic(ofstream& popdyn_annual_indic, int tstep)
{
    ...
    popdyn_annual_indic  << oth_mult << " " << cpue_mult << " "; // output the annual multiplier

    double fbar_py= this->compute_fbar();
    popdyn_annual_indic  << fbar_py << " ";

    double so_far    =this->get_landings_so_far(); //...also including the oth land
    ...
    vector <double>W_at_age=this->get_tot_W_at_age(); // attempt for a calibration to obtain same F
  ...
  vector <double>M_at_age=this->get_tot_M_at_age(); // attempt for a calibration to obtain same F
