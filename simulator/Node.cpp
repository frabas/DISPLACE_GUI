...
//dout << "BEGIN: apply_natural_mortality_at_node()" << endl;
    ...
    {
        a_Ns_at_szgroup[i] =  a_Ns_at_szgroup[i]  *exp(-M_at_szgroup[i]/12); // divide according to tstep
...
// NOTICE THAT THIS IS THE SAME PROCEDURE THAN THE ONE IN do.catch()
    ...
    vector <double> init_Ns_at_szgroup_pop = Ns_at_szgroup_pop; // ...input to be compared with output!
    ...
    vector <double>new_removals_at_szgroup_pop=Ns_at_szgroup_pop; // init
    ...
    double clupeid_sel_ogive [ ] = {0.000601409471228288, 0.00929350798322286, 0.127575084407118,
                                    0.695076124968439, 0.972628235474595, 0.998197939139254, 0.999884202454116, 0.99999257077887,1,1,1,1,1,1
                                   }; // same as for OTM_SPF
    double gadoid_sel_ogive [ ] = {0.00000225976, 0.00001410136387, 0.00008798955470,
    ...
    if(tot!=0)
    {
        vector <double> new_avai_pops_at_selected_szgroup = this->get_avai_pops_at_selected_szgroup(name_pop); // init
        ...
        for(unsigned int szgroup=0; szgroup <avail_biomass.size(); szgroup++)
        {
            if(avail_biomass[szgroup] !=0)
            {
                ...
                if(removals_per_szgroup[szgroup] > new_Ns_at_szgroup_pop[szgroup])
                {
                    ...
                    removals_per_szgroup[szgroup]=new_Ns_at_szgroup_pop[szgroup]; // take all...
                    new_Ns_at_szgroup_pop[szgroup]=0; // nothing left.
                    catch_per_szgroup[szgroup]=catch_per_szgroup[szgroup] *(1-dis_ogive[szgroup]); // just right after the calculation of removals, reverse back to get the landings only
                ...
                else
                {
                    ...
                    catch_per_szgroup[szgroup]=catch_per_szgroup[szgroup] *(1-dis_ogive[szgroup]); // reverse back to get the landings only
