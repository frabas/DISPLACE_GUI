...
class Node
{
    public:
     ...
     /**  destructor */
      ...
      virtual string get_name() const; // declare virtual to enable dynamic binding for chlidren classes e.g. Harbour
      //virtual double get_prices(string met, int pop) ; // declare virtual to enable dynamic binding for chlidren classes e.g. Harbour
      virtual double get_prices_per_cat(int pop, int cat) ; // declare virtual to enable dynamic binding for chlidren classes e.g. Harbour
      virtual double get_fuelprices(string vsize) ; // declare virtual to enable dynamic binding for chlidren classes e.g. Harbour
      double get_x() const;
      double get_y() const;
      ...
      vector<double> get_benthos_tot_biomass() const;
      vector< vector<double> >  get_Ns_pops_at_szgroup() const;
      vector<int> get_vid();
      vector<int> get_pop_names_on_node();
      vector<double> get_impact_on_pops ();
      int get_cumftime() const;
      void set_xy(double xval, double yval);
      void init_Ns_pops_at_szgroup(int nbpops, int nbszgroups);
      void set_Ns_pops_at_szgroup(int name_pop, vector<double> val);
      void set_Ns_pops_at_szgroup_at_month_start(int name_pop, vector<double> val);
      void set_removals_pops_at_szgroup(int name_pop, vector<double> val);
      void set_pressure_pops_at_szgroup(int name_pop, vector<double> val);
      ...
      void apply_natural_mortality_at_node(int name_pop, vector<double> M_at_szgroup);
      void apply_oth_land(int name_pop, double &oth_land_this_pop_this_node, vector<double>  weight_at_szgroup, vector<double> totN);
    ...
    private:
     ...
     vector< vector<double> > pressure_pops_at_szgroup; //i.e. impact
     vector< vector<double> > avai_pops_at_selected_szgroup;
     vector<double> impact_on_pops; // a proportion i.e. ratio removals/available on node
     vector<int> vid; // list of index of vessels currently on the node
     vector<int> pop_names_on_node;
     vector<double> benthos_tot_biomass; // possibly per functional group
