...
bool DbHelper::saveConfig(const Config &cfg)
{
    ...
    QList<int> il = cfg.implicit_pops();
            foreach (int i, il)str.push_back(QString::number(i));
    setMetadata("config::ipops", str.join(" "));

    str.clear();
    QList<double> dl = cfg.calib_oth_landings();
            foreach (double d, dl)str.push_back(QString::number(d));
    ...
    auto ih = cfg.m_interesting_harbours;
            foreach (auto d, ih)str.push_back(QString::number(d.toIndex()));
...
bool DbHelper::loadScenario(Scenario &sce)
{
    ...
    sce.setIs_check_all_stocks_before_going_fishing(
            getMetadata("sce::check_all_stocks_before_going_fishing").toDouble());
...
bool DbHelper::saveScenario(const Scenario &sce)
{
    ...
    setMetadata("sce::check_all_stocks_before_going_fishing",
                (sce.getIs_check_all_stocks_before_going_fishing() ? "1" : "0"));
...
bool DbHelper::loadNodes(QList<std::shared_ptr<NodeData> > &nodes, QList<std::shared_ptr<HarbourData> > &harbours,
                         DisplaceModel *model)
{
...
bool
DbHelper::loadFishFarms(const QList<std::shared_ptr<NodeData> > &nodes, QList<std::shared_ptr<FishfarmData>> &fishfarms)
{
...
bool DbHelper::updateVesselsToStep(int tstep, QList<std::shared_ptr<VesselData> > &vessels)
{
    ...
    std::cout << "Update Vessels Performances: "
