...
// caution: need minGW 4.4 instead of more recent one....
// to do for installation:
// * install minGW 4.4
// * install Qt 4.8.3 for windows mingw
// * put in FRONT of PATH => C:\Qt\4.8.3\bin;C:\Qt\4.8.3\lib
// * put in build options of the code::blocks project
//     > linkers settings > add
//           QtCore4
//           QtGui4
//     > search directories > add
// C:\Qt\4.8.3\include
...
#ifdef _WIN32
#include <windows.h> // caution: windows specific
#include <direct.h> // caution: windows specific, for mkdir
#endif
...
#define MULT_FUELCONS_WHEN_STEAMING 1  // a multiplier, [should be incorporated to the vessel class, later]
#define MULT_FUELCONS_WHEN_FISHING 1.1     // a multiplier [should be incorporated to the vessel class, later]
#define MULT_FUELCONS_WHEN_RETURNING 1.1     // a multiplier [should be incorporated to the vessel class, later]
#define MULT_FUELCONS_WHEN_INACTIVE 0.2  // a multiplier [should be incorporated to the vessel class, later]
#define PI 3.14159265
...
int main(int argc, char* argv[])
{
    ...
    dout << "---------------------------" << endl;
    dout << "---------------------------" << endl;
    dout << " SET UP A Qt SCENE         " << endl;
    dout << "---------------------------" << endl;
    dout << "---------------------------" << endl;

       cout << "coucou here!" << endl;
    ...
    // default
      ...
      // (in code::blocks, see Project>Set program´s arguments in code::blocks menu)
      ...
      string a_semester= "semester1"; // start semester
      ...
      if(!status) cerr << "could not create directory" << namefolder << endl;
  #endif
      ...
      for (int waste=0; waste< a_rand; waste++) {} // to avoid chosing the seed in the same second!
      ...
      {
          int rand_avai_file= (rand() % 50)+1; // pick up a file name randomly later on
          char buffer [100];
      ...
      {
          str_rand_avai_file = "baseline"; // no stochastic variation
      }
    ...
    vector<int> res = do_sample(100, 4, val, p); // draw 100 numbers from these 4 values, see myRutils.cpp
    ...
    for (unsigned int i=0; i< nodes.size(); i++)
    {
        ...
        nodes.at(i)->init_avai_pops_at_selected_szgroup(nbpops,SEL_NBSZGROUP); // caution:magic number...i.e. from the R poisson regression on cpue
    }
    ...
    for(int landscape=0; landscape<nbland; landscape++)
    {
        ...
        for (multimap<int, double>::iterator pos=lower_land; pos != upper_land; pos++)
        {
            ...
            init_tot_biomass_per_group.push_back(pos->second); // biomass per cell for this group specific to this landscape
        }
    ...
    dout << "---------------------------" << endl;
      ...
      for (unsigned int sp=0; sp<populations.size(); sp++)
      {
          ...
          for (multimap<int, int>::iterator pos=lower_init_selsz; pos != upper_init_selsz; pos++)
              init_selected_szgroups.push_back(pos->second); // convert in thousands
          ...
          for (multimap<int, double>::iterator pos=lower_init; pos != upper_init; pos++)
              init_tot_N_per_szgroup.push_back(pos->second * 1000); // convert in thousands
          ...
          if (!binary_search (implicit_pops.begin(), implicit_pops.end(),  sp  ) )
          {
              ...
              for (unsigned int n=0; n< nodes_with_presence.size(); n++)
              {
                  ...
                  {
                      cout << nodes_with_presence.at(n) ; // inconsistence between lst_idx_nodes and avai files if this happen...
                  }
      ...
      */
      ...
      double the_longs1 [] = {6.27136,10.841675,12.863159,12.533569 ,15.587768 }; // x
      ...
      double the_lats2[] = { 54.29481, 54.61786, 54.42403, 55.03321, 55.96545 }; // y
      double the_longs2 [] = { 9.91210, 11.10286, 12.15808, 14.04587, 15.20759}; // x
      ...
      double the_lats3[] = {57.58070, 57.81145, 56.78692, 55.70700, 55.22704, 54.62709, 54.44249, 55.03321, 55.85469 }; // y
      double the_longs3 [] = {8.043674, 11.170626, 11.877336, 10.754344, 11.093178, 10.822111, 12.216170, 14.065234, 15.149502 }; // x
      ...
      vector <Ship*> ships(shipids.size()); //here
      for (int i=0; i<shipids.size(); i++)
      {
          cout<<"create ship " << i << endl;
          if(lane_ids[i]==1) ships[i]= new Ship(shipids[i], vmaxs[i], vcruises[i], longs1, lats1); // North Sea - Øresund - Bornholm
          if(lane_ids[i]==2) ships[i]= new Ship(shipids[i], vmaxs[i], vcruises[i], longs2, lats2); // Kiel-Bornholm
          if(lane_ids[i]==3) ships[i]= new Ship(shipids[i], vmaxs[i], vcruises[i], longs3, lats3); // North Sea - great Belt - Bornholm (RouteT: deep-water transit route in the Danish Waters)
    ...
    // read general vessel features
      ...
      for (int i=0; i<vesselids.size(); i++)
              ...
              //for (int i=0; i<7; i++)
          {
              ...
              {
                  vector<int> one_harbour = do_sample(1, spe_harbours.size(), &spe_harbours[0], &spe_freq_harbours[0]); // need to convert in array, see myRutils.cpp
              ...
              else
              {
                  ...
                  start_harbour=find_entries_s_i(harbours, vesselids[0])[0]; // CAUTION: LIKE A MAGIC NUMBER HERE!!!
        ...
        vessels[i]= new VesselGUI(nodes[start_harbour],
                                  i,
                                                                vesselids[i],
                                                                nbpops,
                                                                NBSZGROUP,
        ...
        // create a vessel-like for the Qt scene
        vessels[i]->setScale(qreal(0.5)); // size of the item
        mw->scene()->addItem(vessels[i]);


        // for dyn sce. CAUTION: MAGIC NUMBERS HERE FOR SOME SCENARIOS....
           if (binary_search (dyn_alloc_sce.begin(), dyn_alloc_sce.end(), "reduced_speed_10percent")) // dyn sce.
           {
               vessels[i]->set_speed(  vessels[i]->get_speed()*0.9   ); // a decrease by 10%...
               vessels[i]->set_fuelcons( vessels[i]->get_fuelcons()*0.7 ); // corresponds to a decrease by 30% in fuelcons
               // cubic law  c=v^3, see Ronen 1982
               // e.g. assuming a v at 10, the fuel conso is lowered by (in %) =>  (1- (((seq(0.1,1,by=0.1)*10)^3 ) / (1*10^3)) )*100
           }
           if (binary_search (dyn_alloc_sce.begin(), dyn_alloc_sce.end(), "reduced_speed_20percent")) // dyn sce.
           {
               vessels[i]->set_speed(  vessels[i]->get_speed()*0.8   ); // a decrease by 20%...
               vessels[i]->set_fuelcons( vessels[i]->get_fuelcons()*0.512 ); // corresponds to a decrease by 48.8% in fuelcons
               // cubic law  c=v^3, see Ronen 1982
               // e.g. assuming a v at 10, the fuel conso is lowered by (in %) =>  (1- (((seq(0.1,1,by=0.1)*10)^3 ) / (1*10^3)) )*100
           }
           if (binary_search (dyn_alloc_sce.begin(), dyn_alloc_sce.end(), "reduced_speed_30percent")) // dyn sce.
           {
               vessels[i]->set_speed(  vessels[i]->get_speed()*0.7   ); // a decrease by 30%...
               vessels[i]->set_fuelcons( vessels[i]->get_fuelcons()*0.343 ); // corresponds to a decrease by 65.7% in fuelcons
               // cubic law  c=v^3, see Ronen 1982
               // e.g. assuming a v at 10, the fuel conso is lowered by (in %) =>  (1- (((seq(0.1,1,by=0.1)*10)^3 ) / (1*10^3)) )*100
           }
           ...
           // a particular setters for the CPUE STUFF...
             ...
             vessels.at(i)->init_gshape_cpue_nodes_species(nbnodes, nbpops); // init the vector of vector with Os
             vessels.at(i)->init_gscale_cpue_nodes_species(nbnodes, nbpops); // init the vector of vector with Os
             for (int n=0; n< gshape_name_nodes_with_cpue.size(); n++)
             {
                 vector<double> gshape_cpue_species = find_entries_i_d (gshape_cpue_per_stk_on_nodes, gshape_name_nodes_with_cpue[n]); // look into the multimap...
                 vector<double> gscale_cpue_species = find_entries_i_d (gscale_cpue_per_stk_on_nodes, gshape_name_nodes_with_cpue[n]); // look into the multimap...
                 if(!gshape_cpue_species.empty())
                 {
                     vessels.at(i)->set_gshape_cpue_nodes_species(n, gshape_cpue_species); // caution here: the n is the relative index of the node for this vessel i.e. this is not the graph index of the node (because it would have been useless to create a huge matrix filled in by 0 just to preserve the graph idex in this case!)
                     vessels.at(i)->set_gscale_cpue_nodes_species(n, gscale_cpue_species); // caution here: the n is the relative index of the node for this vessel i.e. this is not the graph index of the node (because it would have been useless to create a huge matrix filled in by 0 just to preserve the graph idex in this case!)
                 }
             }
             ...
             // need to compute expected cpue (averaged over node but cumulated over species)
                  ...
                  for(int pop = 0; pop < nbpops; pop++)
                  {
                      ...
                      expected_cpue_this_pop.at(pop)=0; // init

                      // compute cpue on nodes
                      for(int f = 0; f < fgrounds.size(); f++)
                      {
                          double a_shape = gshape_cpue_nodes_species.at(f).at(pop); // look into the vector of vector....
                          double a_scale = gscale_cpue_nodes_species.at(f).at(pop); // look into the vector of vector....
                      ...
                      if(expected_cpue_this_pop.at(pop)!=0) expected_cpue_this_pop.at(pop)= expected_cpue_this_pop.at(pop)/fgrounds.size(); // do the mean

                      expected_cpue+= expected_cpue_this_pop.at(pop); // sum over pop
                  }
                  ...
                  for(int f = 0; f < fgrounds.size(); f++)
                  {
                      ...
                      for(int pop = 0; pop < nbpops; pop++)
                      {
                          cumcatch_fgrounds_per_pop[f][pop] = 0; // init
                  ...
                  vessels.at(i)->compute_experiencedcpue_fgrounds(); // compute for the first time, to get freq_experiencedcpue_fgrounds...
                  // ...or per pop
                  ...
                  vessels.at(i)->compute_experiencedcpue_fgrounds_per_pop(); // compute for the first time, to get freq_experiencedcpue_fgrounds_per_pop...
  ...
  dout << "---------------------------" << endl;
    ...
    for(int i=0; i<graph_idx_dep.size(); i++) // col 1
    {
    ...
    for(int i=0; i<graph_idx_dep.size(); i++) // col 2
    {
    ...
    if(do_it) // test
    {
        ...
        DijkstraComputePaths(origin, adjacency_map, min_distance, previous, dest); // from the source to all nodes
        ofstream paths;
        ...
        int idx = it - idx_paths_shop.begin(); // tricky!
    ...
    dout << "---------------------------" << endl;
       ...
       all_harbour_nodes.erase( all_harbour_nodes.begin() ); // erase first element which is blank....
       // quarter2
       ...
       all_fgrounds_nodes.erase( all_fgrounds_nodes.begin() ); // erase first element which is blank....
       // quarter2
       ...
       vector<int> relevant_nodes (all_harbour_nodes); // copy
       ...
       else
       {
           ...
           min_distance.clear();
           previous.clear();
           //for (int i=3000; i<relevant_nodes.size(); i++) // change for this to debug in case the creation fails...
           for (int i=0; i<relevant_nodes.size(); i++)
           {
               cout << ".";
               dout << "i: "<< i << "max size: " << relevant_nodes.size() << endl;

               if(read_preexisting_paths) // this is a program´s argument option
               {
               ...
               else
               {
                   ...
                   DijkstraComputePaths(relevant_nodes.at(i), adjacency_map, min_distance, previous, relevant_nodes); // from the source to all nodes
    ...
    // realtime gnuplot
        if(use_gnuplot)
        {
            char *path = "\"C:\\Program Files (x86)\\gnuplot\\bin\\gnuplot\"";  // for gnuplot installed for at least MinGW_with_gcc_4.6.2
            pipe2 = _popen(path, "w");
            ...
            pipe3 = _popen(path, "w"); //this opens gnuplot
            pipe4 = _popen(path, "w"); //this opens gnuplot
            ...
            if(namefolderinput=="final" || namefolderinput=="fake") // i.e. canadian
            {
        ...
        cout << "create the suite of output files" << endl;
        ...
        for (int sp=0; sp<populations.size(); sp++)
        {
            ...
            populations.at(sp)->aggregate_N(); // get total N from summing up N over nodes
            ...
            {
                popdyn_N  << tot_N_at_szgroup.at(sz) / 1000 << " " ; // output in thousands of individuals
            }
            popdyn_N << " " <<  endl;
        }
    ...
    for (int tstep =0; tstep < nbsteps; ++tstep)
    {
        ...
        // Qt related code
        mw->scene()->advance();
        // sleep to prevent the event loop eats 100% cpu
        Sleep(0);
        app.processEvents();
        ...
        if(use_gnuplot)
            ...
            // init for SMS
            int myints[] = {10,3,7}; // reuse the order of sp in the SMS file (i.e. cod, herring, sprat)
            vector<int> stock_numbers (myints, myints + sizeof(myints) / sizeof(int) );
            int myints2[] = {1000,1000,10000}; //  cod and herring in thousands, sprat in thousands and 10% in western baltic
            vector<int> some_units (myints2, myints2 + sizeof(myints2) / sizeof(int) );
            int myints3[] = {8,8,8}; //  cod and herring in thousands, sprat in thousands and 10% in western baltic
            vector<int> some_max_nb_ages (myints3, myints3 + sizeof(myints3) / sizeof(int) );

            if(binary_search (dyn_pop_sce.begin(), dyn_pop_sce.end(), "use_SMS"))  // entry point for the SMS model
            {
        ...
        //----------------------------------------//
            ...
            // CHECK...CHECK...CHECK...
                 ...
                 if(namefolderinput=="fake")
                 {
                     sample_pops.push_back(0); // for pop xx
                     sample_pops.push_back(1); // for pop xx
                     sample_pops.push_back(2); // for pop xx
                 }
                 else
                 {
                     sample_pops.push_back(15); // for pop xx
                 }
                 for (unsigned int sp=0; sp<sample_pops.size(); sp++)
                 {
                     dout << "write down re-read pop in the popdyn_test2 file for checking...";
                     populations.at( sample_pops.at(sp) )->aggregate_N(); // get total N from summing up N over nodes
                     ...
                     {
                         popdyn_test2  << tot_N_at_szgroup.at(sz) / 1000 << " " ; // output in thousands of individuals
                 ...
                 if(binary_search (tsteps_months.begin(), tsteps_months.end(), tstep))  // apply M only by month to save computation time...
                 {
                     ...
                     for (unsigned int sp=0; sp<populations.size(); sp++)
                     {
                         if (!binary_search (implicit_pops.begin(), implicit_pops.end(),  sp  ) )
                         {
                             ...
                             double cumul_oth_land_to_be_displaced=0.0; // will be used in case of area_closure
                             ...
                             for(unsigned int n=0; n<a_list_nodes.size(); n++)
                             {
                                 ...
                                 if (binary_search (dyn_pop_sce.begin(), dyn_pop_sce.end(), "with_stochast_oth_land")) // area-based sce
                                 {
                                     double a_rnorm = rlnorm(0,0.25); // lognormal error
                                 ...
                                 if(oth_land_this_pop_this_node!=0)
                                 {
                                     ...
                                     // caution: quick and dirty solution because we don´t know on which node
                                                        ...
                                                        "area_closure")) // area-based sce
                                     {
                                         ...
                                         if (binary_search (polygon_nodes.begin(), polygon_nodes.end(), a_list_nodes.at(n)->get_idx_node()))
                                         {
                                             ...
                                             a_source_node_idx = n; // the relative node idx with oth_land to be displaced....
                                         ...
                                         else
                                         {
                                             ...
                                             if(dist_to_this_node<200)  // MAGIC NUMBER 200 km
                                             {
                                                 ...
                                                 cumul_oth_land_to_be_displaced=0.0; // reinit
                                             } // sinon, passe ton chemin....

                                             // no this is not a closed node...so apply oth_land
                                             // but also adding the previous cumul if suitable!
                                             vector <double> totN = populations.at(name_pop)->get_tot_N_at_szgroup(); // needed to impact the availability
                             ...
                             for (unsigned int n=0; n<a_list_nodes.size(); n++)
                             {
                                 ...
                                 for(int szgroup=0; szgroup<N_at_szgroup_at_month_start.size(); szgroup++)
                                 {
                                     ...
                                     // 1- per szgroup
                                     pressure_per_szgroup_pop[szgroup]=removals_per_szgroup[szgroup]/N_at_szgroup_at_month_start[szgroup]; // the pressure is a proportion
                                     a_list_nodes.at(n)->set_pressure_pops_at_szgroup(  name_pop, pressure_per_szgroup_pop);

                                     tot_removals+=(removals_per_szgroup[szgroup]*wsz[szgroup]); // cumul
                                     tot_B+=(N_at_szgroup_at_month_start[szgroup]*wsz[szgroup]); // cumul
                                 }
                                 ...
                                 if(tot_removals!=0)
                                 {
                                     ...
                                     if(tot_B!=0)
                                     {
                                         if(tot_removals!=0)
                                         {
                                             impact_on_pop=tot_removals/tot_B; // this is a proportion (of biomass)
                                         ...
                                         {
                                             impact_on_pop=-1; // =>  available biomass but no catch...
                                         }
                                     }
                                     else
                                     {
                                         impact_on_pop=0; // => no available biomass then no catch...
                                     }
                                 ...
                                 a_list_nodes.at(n)->clear_removals_pops_at_szgroup(); // RE-INIT
                         ...
                         for (unsigned int sp=0; sp<populations.size(); sp++)
                         {
                             if (!binary_search (implicit_pops.begin(), implicit_pops.end(),  sp  ) )
                             {
                                 ...
                                 if(use_gnuplot)
                                 {
                                     ...
                                     fprintf(pipe3, command.c_str()); //plotting the .dat file
                                     ...
                                     fprintf(pipe4, command2.c_str()); //plotting the .dat file
                                 ...
                                 if(binary_search (tsteps_years.begin(), tsteps_years.end(), tstep)) // apply only at the beginning of the year (this is maybe not always relevant...)
                                 {
                                     ...
                                     populations.at(sp)->set_cpue_multiplier(pow(sum_N_start_current_year/sum_N_year_minus_1, 0.7)); // MAGIC NUMBER HERE! see Harley et al 2001 Canadian Journal
                                 ...
                                 if(binary_search (tsteps_semesters.begin(), tsteps_semesters.end(), tstep)) // apply only by semester, to be consistent with timetable of survey data
                                 {
                     ...
                     for (unsigned int sp=0; sp<populations.size(); sp++)
                     {
                         if (!binary_search (implicit_pops.begin(), implicit_pops.end(),  sp  ) )
                         {
                             ...
                             populations.at(sp)->export_popdyn_N (popdyn_N, tstep); // N at szgroup
                             populations.at(sp)->export_popdyn_F (popdyn_F, tstep); // ...and F at age
                             ...
                             if(binary_search (tsteps_years.begin(), tsteps_years.end(), tstep))          // apply only at the beginning of the year (this is maybe not always relevant...)
                             {
                                 ...
                                 if (!binary_search (implicit_pops.begin(), implicit_pops.end(),  namepop  ))
                                 {
                                     ...
                                     if(populations.at(sp)->get_oth_land_multiplier()!=
                                             populations.at(sp)->get_oth_land_multiplier()) // i.e. trick to check if nan
                                     {
                                     ...
                                     // export
                                     populations.at(sp)->export_popdyn_annual_indic(popdyn_annual_indic, tstep); // ...export the cpue and oth_land multiplier
                                 ...
                                 if(binary_search (dyn_pop_sce.begin(), dyn_pop_sce.end(), "use_SMS"))  // store N initial for the next year and reinit Fs
                                 {
                 ...
                 if(tstep>2000 && binary_search (tsteps_quarters.begin(), tsteps_quarters.end(), tstep)) // EVENT => change of quarter
                     //   if(tstep==3 || tstep==4) // use this to start from another quarter if test...
                 {
                     ...
                     // RE-read general vessel features: don´t forget to clear the vectors!
                     ...
                     for (unsigned int v=0; v<vessels.size(); v++) // LOOP OVER VESSELS
                     {
                         ...
                         if(spe_harbours.empty())
                         {
                             // if missing info for a given vessel for this quarter
                             cout << "no specified harbour in this quarter for this vessel..." << endl;
                             int start_harbour=vessels.at(v)->get_harbours()[0]; // CAUTION: TAKE FROM PREVIOUS QUARTER!
                             spe_harbours.push_back(start_harbour);
                             spe_freq_harbours.push_back(1);
                             cout << "then take node: " << start_harbour << endl;
                         }
                         ...
                         // init cpue_nodes_species for this vessel
                         int nbnodes=gshape_name_nodes_with_cpue.size();
                         vessels.at(v)->init_gshape_cpue_nodes_species(nbnodes, nbpops); // init the vector of vector with Os
                         vessels.at(v)->init_gscale_cpue_nodes_species(nbnodes, nbpops); // init the vector of vector with Os
                         for (int n=0; n< gshape_name_nodes_with_cpue.size(); n++)
                         {
                             vector<double> gshape_cpue_species = find_entries_i_d (gshape_cpue_per_stk_on_nodes, gshape_name_nodes_with_cpue[n]); // look into the multimap...
                             vector<double> gscale_cpue_species = find_entries_i_d (gscale_cpue_per_stk_on_nodes, gshape_name_nodes_with_cpue[n]); // look into the multimap...
                             if(!gshape_cpue_species.empty())
                             {
                                 vessels.at(v)->set_gshape_cpue_nodes_species(n, gshape_cpue_species); // caution here: the n is the relative index of the node for this vessel i.e. this is not the graph index of the node (because it would have been useless to create a huge matrix filled in by 0 just to preserve the graph idex in this case!)
                                 vessels.at(v)->set_gscale_cpue_nodes_species(n, gscale_cpue_species); // caution here: the n is the relative index of the node for this vessel i.e. this is not the graph index of the node (because it would have been useless to create a huge matrix filled in by 0 just to preserve the graph idex in this case!)
                             }
                         }
                         ...
                         for(int pop = 0; pop < nbpops; pop++)
                         {
                             ...
                             {
                                 double a_shape = gshape_cpue_nodes_species.at(g).at(pop); // look into the vector of vector....
                                 double a_scale = gscale_cpue_nodes_species.at(g).at(pop); // look into the vector of vector....
                             ...
                             expected_cpue_this_pop.at(pop)= expected_cpue_this_pop.at(pop)/fgrounds.size(); // do the mean
                         ...
                         for(int g = 0; g < fgrounds.size(); g++)
                         {
                             ...
                             for(int pop = 0; pop < nbpops; pop++)
                             {
                                 a_cumcatch_fgrounds_per_pop[g][pop] = 0; // init
                         ...
                         vessels.at(v)->compute_experiencedcpue_fgrounds(); // compute for the first time, to get freq_experiencedcpue_fgrounds...
                         // ...or per pop
                         ...
                         vessels.at(v)->compute_experiencedcpue_fgrounds_per_pop(); // compute for the first time, to get freq_experiencedcpue_fgrounds_per_pop...
                         ...
                         //
                         vessels.at(v)->set_previous_harbour_idx(0); // to force re-computation of the fuel saving scenario
                         ...
                         vessels.at(v)->receive_message(1); // message 1 means: "please, change of grounds as soon as possible"
                 ...
                 if(is_re_read_pop_data) // EVENT => re-read pop data
                 {
                     ...
                     for (int sp=0; sp<populations.size(); sp++)
                     {
                         if (!binary_search (implicit_pops.begin(), implicit_pops.end(),  sp  ) )
                         {
                             ...
                             for(unsigned int sz = 0; sz < tot_N_at_szgroup.size(); sz++)
                             {
                                 popdyn_test  << tot_N_at_szgroup.at(sz) / 1000 << " " ; // output in thousands of individuals
                             }
                             popdyn_test << " " <<  endl;
                         }
                     }
                     ...
                     for (unsigned int i=0; i<populations.size(); i++)
                     {
                         if (!binary_search (implicit_pops.begin(), implicit_pops.end(),  i  ) )
                         {
                             ...
                             populations.at(i)->distribute_N(); // divide on nodes according to avai
                 ...
                 random_shuffle(ve.begin(),ve.end()); // random permutation
                 ...
                 ///------------------------------///
                      ...
                      ///------------------------------///
                      ///------------------------------///
                      for (unsigned int idx_v =0; idx_v < ve.size(); idx_v++) // LOOP OVER VESSELS
                      {
                          ...
                          if(vessels[ index_v ]->get_roadmap().empty())
                          {
                              ...
                              if(vessels[ index_v ]->get_possible_metiers().size()>1) // when at least one possible metier within this quarter
                              {
                                  ...
                                  if(vessels[ index_v ]->get_inharbour())
                                  {
                                      ...
                                      if(!vessels[ index_v ]-> get_inactive()) // i.e. just arrived!
                                      {
                                  ...
                                  else
                                  {
                                      ...
                                      if(vessels[ index_v ]->read_message()==1) // check my mailbox: Am I forced to change of ground?...
                                      {
                                          if(vessels[ index_v ]->get_fgrounds().size()<3)
                                          {
                                              stop_fishing=true; //in this case, forced to go back to port instead!
                                          }
                                          force_another_ground=true; //in this case, forced to change!
                                          // reset my mail box
                                          vessels[ index_v ]->reset_message();
                                      }

                                      // ***************implement a decision************************************
                                      if(!stop_fishing)  //go on fishing...
                                      {
                                          // ***************make a decision************************************
                                          ...
                                          if(another_ground || force_another_ground ) // ...but not on this ground!
                                          {
                          ...
                          if(export_vmslike && tstep<8641) if( vessels[ index_v ]->get_state()!=3)
                              {
                                  vmslike << tstep << " "
                                          ...
                                          << vessels[ index_v ]->get_tstep_dep() << " " // can be used as a trip identifier
                      ...
                      if(use_gnuplot)
                      {
                          ...
                          fprintf(pipe2, command1.c_str()); //plotting the .dat file
