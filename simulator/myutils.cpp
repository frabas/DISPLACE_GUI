                          ...
                          std::vector<int> relevant_nodes)
{
    ...
    while (!relevant_nodes.empty() && !vertex_queue.empty())
    {
                ...
                edge_iter++)
        {
            ...
            weight_t weight = edge_iter->weight; // GET HERE THE DIST IN km (to the nearest km because an integer here)...
            //relax the edge (u,v)>>=
    ...
    string inputfolder)
{
    ...
    for (int i=0; i<relevant_nodes.size(); i++)
    {
        ...
        paths.insert (paths.end(),path.begin(),path.end()); // concatenate
                      ...
                      vector<int>& graph_dist_km, int nrow)
{
    ...
    {
        in >> val;
        unsigned int val_int = static_cast<unsigned int>(val + 0.5); // caution: convert into imt to speedup c++
                       ...
                       map<string, int>& external_states, map<string, int>& internal_states)
{
    ...
    for (int i=0; i<direction.size(); i++)
    {
        ...
        {
            a_state = external_states.find(a_delimiter)->second; // get the current status of the drivers....
        }
        else
        {
            a_state = internal_states.find(a_delimiter)->second; // external not found, search for internal states instead.
        }
        //cout << "a tree " << tree << endl;
        //cout << "a delimiter " << a_delimiter << endl;
        //cout << "a state " << a_state << endl;
        split( a_split_string, tree, a_delimiter, splitX::no_empties); // split the tree in 2 according to the driver
        //print(a_split_string);
        //   system("PAUSE");
        if(a_split_string.size()>1) tree = a_split_string[a_state]; // keep only one branch (0 left or 1 right) and loop...
