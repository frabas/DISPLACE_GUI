...
private:
    ...
    vector<double> freq_fgrounds; // freq of visit per node
    vector<double> freq_harbours; // freq of visit per node
    vector<double> cumcatch_fgrounds; // cumulated catch per node for the last trip, reinit when new trip start
    vector< vector<double> > cumcatch_fgrounds_per_pop;
    vector<double> cumeffort_fgrounds; // cumulated effort per node for the last trip, reinit when new trip start
    vector<double> experiencedcpue_fgrounds; // from cumcatch/cumeffort for last trip
    vector<vector<double> > experiencedcpue_fgrounds_per_pop; // from cumcatch/cumeffort for last trip
    vector<double> freq_experiencedcpue_fgrounds; // scaled to 1 from experiencedcpue_fgrounds
    vector<vector<double> > freq_experiencedcpue_fgrounds_per_pop; // scaled to 1 from experiencedcpue_fgrounds
    vector<double> vessel_betas_per_pop; // skipper effect, target factor
    vector<double> percent_tac_per_pop; //
    vector<int> idx_used_metiers_this_trip; // clear at the end of the trip
    multimap <int, int> possible_metiers; // list of possible metiers on specific ground
    multimap <int, double> freq_possible_metiers; // freq of the metier on specific fground
    ...
    double x, y, prev_x, prev_y, next_x, next_y, course; // because movement is semi-continuous...
    double speed, fuelcons, length, KW, tankcapacity, carrycapacity, nbfpingspertrip, resttime_par1, resttime_par2, av_trip_duration; // fixed
    double timeforrest, cumfuelcons, consotogetthere, cumsteaming, distprevpos, timeatsea, traveled_dist_this_trip, cumcatches, reason_to_go_back;  // dynamic
    string length_class, nationality;
    int message;
    int state;
    int tstep_dep;
    int previous_harbour_idx;
    bool inharbour, inactive, natio; // dynamic
...
public:
    ...
    int should_i_go_fishing(map<string, int>& external_states, bool use_the_tree); //yes:1; no=0
    ...
    int should_i_stop_fishing(map<string, int>& external_states, bool use_the_tree, //yes:1; no=0
                                  int tstep,
                                  vector<string>& dyn_alloc_sce,
                                  int create_a_path_shop,
                                  vector <int>& idx_path_shop,
