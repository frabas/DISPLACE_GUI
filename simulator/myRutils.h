...
void ProbSampleReplace(int n, double *p, int *perm, int nans, int *ans); // in random.c
...
inline vector<double> destB (double x1, double y1, double angleAB, double distkm)
{
    ...
    double p = 180/PI;
    double Rearth = 6371.0;
    res[1] =  asin((sin(y1/p)*cos(distkm/Rearth)) + (cos(y1/p)*sin(distkm/Rearth)*cos(angleAB/p))) ; // LAT
    res[0] =  (x1/p + atan2(sin(angleAB/p)*sin(distkm/Rearth)*cos(y1/p), cos(distkm/Rearth)-sin(y1/p)*sin(res[1])) ) ; // LONG
    res[1] =   res[1] *p;
    res[0] =   res[0] *p;
    return(res);
}
